...
fase_grupos %>%
  ...
  group_by(PAIS_A) %>%
  mutate(SIMU_GOLS_FEITOS_A1 = sum(GOLS_A),
         SIMU_GOLS_SOFRIDOS_A1 = sum(GOLS_B)) %>%
  group_by(PAIS_B) %>%
  mutate(SIMU_GOLS_FEITOS_B1 = sum(GOLS_B),
         SIMU_GOLS_SOFRIDOS_B1 = sum(GOLS_A)) %>%
  ungroup() %>%
...
```{r}
fase_grupos %>%
  filter(GRUPO_A == GRUPO_B) %>%
  mutate(GOLS_A = rpois(n(), LAMBDA_AB),
         GOLS_B = rpois(n(), LAMBDA_BA)) %>% 
  group_by(GRUPO_A) %>%
  group_modify(~{
    .x %>%
      select(PAIS_A, PAIS_B, GOLS_A, GOLS_B) %>%
...
```{r}
  ...
  # Placares simulados e pontuações de cada equipe
  df_resultado <- fase_grupos %>%
    filter(GRUPO_A == "G", GRUPO_B == "G") %>%
    mutate(GOLS_A = rpois(n(), LAMBDA_AB),
           GOLS_B = rpois(n(), LAMBDA_BA)) %>%
  ...
  brasil_venceu_pais <- (df_resultado %>%
    filter(PAIS_A == "Brazil",
           PAIS_B == pais_i,
           GOLS_A > GOLS_B) %>%
    nrow() > 0)
  
  # Países selecioandos para a próxima fase
  df_selecionados <- df_resultado %>%
  ...
  brasil_foi_selecionado <- ("Brazil" %in% pull(df_selecionados, PAIS))
  
  # print(df_resultado)
  # print(df_selecionados)
  # cat(pais_i, brasil_venceu_pais, brasil_foi_selecionado, "\n", sep="\t")
```
